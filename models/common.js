...
const getHrJobPending = async (req) => {
  ...
  let sqlQuery = `SELECT * FROM (SELECT JD.ID as jdId, J.job_number, DC.category_name, S.id as section_id, S.section_name, o.order_number, O.fabric_yarn_name, O.color_name, WS.type_name, D.defect_name, J.REMARKS, ISNULL(ID.qty, 0) as invoiceQty, JD.*, CONVERT(varchar, created_date, 23) as created_date FROM JOBS J INNER JOIN jobDetails JD ON JD.job_id = J.ID INNER JOIN sections S ON S.ID = J.section_id
